// COLLYWOOD — Fonction CinetPay (Supabase Edge Function)
// /init   : appelé par l'app -> crée l'abonnement en attente + renvoie l'URL de paiement CinetPay
// /notify : appelé par CinetPay après paiement -> vérifie et ACTIVE l'abonnement automatiquement
...
const APP_URL  = "https://collywood.vercel.app/v2.html";   // à changer vers le domaine final plus tard
...
Deno.serve(async (req) => {
  ...
  if (path === "init") {
    ...
    const { data: sub, error: e1 } = await SB.from("subscriptions")
      .insert({ user_id: user.id, plan_id: "mois", status: "pending" })
      .select().single();
    if (e1) return json({ error: e1.message }, 400);

    const { data: pay, error: e2 } = await SB.from("payments")
      .insert({ user_id: user.id, subscription_id: sub.id, amount_usd: AMOUNT, method: "mobile_money", status: "pending" })
      .select().single();
    ...
    const r = await fetch("https://api-checkout.cinetpay.com/v2/payment", {
      ...
      body: JSON.stringify({
        ...
        amount: AMOUNT,
        currency: CURRENCY,
        description: "Abonnement COLLYWOOD 1 mois",
  ...
  // ═══ 2) NOTIFY : CinetPay confirme -> activation ═══
  if (path === "notify") {
    ...
    // Toujours re-vérifier auprès de CinetPay (sécurité)
